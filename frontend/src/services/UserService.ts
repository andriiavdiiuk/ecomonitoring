...
function decodeJwt(token: string): JwtPayload | null {
 ...
 class UserService {
    ...
    public async register(username: string, email: string, password:string, rememberMe:boolean): Promise<string> {
        const response = await api.post<TokenResponse>("/user/register",{
            username, email, password
        });

        const token = response.data.token
        this.set_cookie(token, rememberMe);
    ...
    private set_cookie(token: string, rememberMe:boolean)
    {
        if (rememberMe) {
            ...
            if (decodedPayload) {
                ...
                this.cookies.set('jwt', token, {expires: expireDate, secure: true, sameSite: 'strict'})
            }
            else
            {
                this.cookies.set('jwt', token, {secure: true, sameSite: 'strict'})
            }
        }
    }
